...
export const SETTINGS_ENTRIES: ManualEntry[] = [
  {
    ...
    items: [
      ...
      ['■ Stop (all)', 'Stops those three where they are. It acts every time you press it, so it also catches a widget that came up running on its own, such as an LFO.'],
      ['↺ Reset (all)', 'Sends those three back to their start point and resets tap averaging. Whatever was playing keeps playing, from the beginning. There are two Reset buttons: a large one next to Play, and a small one in the BPM strip.'],
      ['Timeline and Sound Player stand apart', 'None of the three buttons touch them. A show and its audio run from their own transports, so putting the desk into play, stopping it or resetting it cannot start, halt or rewind them. Drive them from their own buttons, from a cue, or from a router link.'],
      ['Per-widget transport still works', 'Each widget keeps its own play/stop buttons and its own MIDI/OSC mappings. The three top-bar buttons only add a way to move them all at once.'],
      ['Mapping the three buttons', 'Right-click Play, Stop or Reset and pick Learn MIDI or Learn OSC, then send the message. Hovering shows what each one is currently bound to; Esc cancels an armed learn. They can also be set by hand in Settings, Tempo, next to the Tap source. A note fires on press only, so releasing the pad does not trigger a second time.'],
...
export const WIDGET_ENTRIES: ManualEntry[] = [
  ...
  },
  {
    ...
    items: [
      ...
      ['Readability', 'Text follows the same size as the Router, and the numbers are light on dark instead of near-black. The threshold sliders are sized for a fingertip.'],
      ['Scrolling', 'Each section takes the height it needs and the widget scrolls, so nothing is ever silently cut off. Make the widget taller until the scrollbar goes away.'],
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ['Input: widget', 'A cell from any widget on any page — the dropdown groups them by page. Any widget that has cells can be a source or a target, so the Keyboard, Cues, Submasters and Master Level are all in the list. A widget with nothing in it yet (a Keyboard with no keys bound, a Cues with no cues) has no cells and stays out until you add some.'],
