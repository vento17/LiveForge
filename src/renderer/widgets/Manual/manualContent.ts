...
export const SETTINGS_ENTRIES: ManualEntry[] = [
  {
    ...
    items: [
      ...
      ['↺ Reset', 'Resets tap averaging and every widget. There are two: a large one next to Play in the main bar, and a small one in the BPM strip. Same action.'],
      ['Pages', 'The numbered tabs in the middle of the bar. In Config mode drag a tab to reorder the pages (up to 12), and right-click one to delete that page — the − button only ever removes the last one. Deleting is undoable with Ctrl+Z. Reordering is cosmetic: links, cue scopes and router rows all point at pages by identity, so nothing breaks when they move.'],
      ['Live params panel', 'The arrow in the top-right corner of Live mode opens the collapsible parameter panel for the selected widget. Panel and log badge both live on the right-hand edge, so a hand resting on the left of a touchscreen stays clear of them.'],
  ...
  },
  {
    ...
    items: [
      ...
      ['Learn MIDI CC', 'Right-click any cell in Live mode and pick Learn. It asks which Router to put the row in, then the next CC or note received becomes a Router row driving that cell.'],
      ['LEARN MIDI / LEARN OSC', 'The two small buttons next to Load turn on learn mode for the whole surface, so a desk can be patched without right-clicking anything — which matters on a touchscreen, where there is no right-click. Pick a Router when you switch it on and every row of that pass lands in it. Then: tap a cell, move the physical control, tap the next cell, and so on. Tapping the armed cell again disarms it, Esc leaves learn mode.'],
      ['While learn mode is on', 'Linkable cells show a dashed outline and the armed one pulses. Taps on cells only arm them — a button will not fire and a fader will not move, so you can patch a live show without sending anything. Everything else on screen still works normally.'],
  ...
  },
  {
    ...
    items: [
      ...
      ['Esc (in learn mode)', 'Leaves learn mode. It does not leave Live mode, so a second Esc is needed for that.'],
...
export const WIDGET_ENTRIES: ManualEntry[] = [
  ...
  },
  {
    ...
    items: [
      ...
      ['Behaviour survives a link', 'A pad on a controller sends press then release. The link passes that on as a press, and the CELL decides what it means — so a linked Toggle stays lit when you let the pad go, and a linked Radio still releases its neighbours. The controller no longer dictates the behaviour.'],
      ['Lighting the controller LED', 'Every state change sends the cell mapping outward, so a controller patched to the MIDI output follows along: a toggle sends its on value and then nothing until you switch it off. Point the cell at the note your pad lights on, and the LED tracks the real state rather than your finger.'],
    ],
  ...
  },
  {
    ...
    items: [
      ...
      ['Amplitude', 'How far the output swings. At 0 the wave is flat and the output is just the offset.'],
      ['Offset', 'The centre the swing sits around — moves the value UP and DOWN.'],
      ['Phase', 'Where in the cycle you are — moves the wave LEFT and RIGHT in time. Different axis from offset: frozen at one instant, changing phase walks you along the wave, changing offset lifts the whole wave.'],
  ...
  },
  {
    ...
    note: 'Stop freezes time, not the parameters: with the LFO stopped, moving offset, phase, amplitude or waveform still moves the output. Same for the Graph (edit the curve) and the Step Sequencer (edit the step under the playhead).',
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      ['Scrolling', 'A router with more rows than fit its box scrolls vertically, with a visible bar you can drag on a touchscreen.'],
      ['Readout', 'Every row shows the value it received and what each output sends, MIDI and OSC rows included. Names resolve across all pages, so a target on another page reads by name rather than as a question mark.'],
      ['A link does not lock the cell', 'A slaved fader, knob, pad or cue is still playable by hand — the red outline says it is linked, not that it is disabled. Hand control and the link share the cell, so with a MIDI or OSC link you drive it between messages; with a link from an LFO or another running widget, the next tick simply overwrites what you moved.'],
    ],
  ...
  },
  {
    ...
    items: [
      ...
      ['Flash', 'Momentarily takes the scene to full while held.'],
      ['Flash is a cell', 'Each flash button is a cell of its own: right-click it to link or Learn, and a pad on a controller then holds the flash exactly like a finger does. The faders keep cells 0…n-1 and the flash buttons take n…2n-1, so links saved before this still point at the same fader.'],
      ['⚡ Flash output', 'In the Inspector, the ⚡ next to a scene gives its flash button a MIDI/OSC mapping, sent on press and again on release — so a controller patched to the output can light the pad while the flash is held.'],
